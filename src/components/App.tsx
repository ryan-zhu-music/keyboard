...
const Keyboard: React.FC<Props> = ({
  ...
  if ([height, whiteKeyWidth, blackKeyWidth, blackKeyHeight].some((n) => n < 1)) {
    throw new Error('All dimensions must be greater than 0')
  }

  // startNote and endNote must be white keys
  const whiteKeys = allWhiteKeys.slice(allWhiteKeys.indexOf(startNote), allWhiteKeys.indexOf(endNote) + 1)
  ...
  const numberOfKeys = whiteKeys.indexOf(endNote) - whiteKeys.indexOf(startNote) + 1
        ...
        {blackKeys.map((key, index) => (
          <button
            style={{
              ...
              marginRight:
                ['D', 'A'].includes(key.charAt(0)) && index !== blackKeys.length - 1
        ...
        {whiteKeys.map((key, index) => (
          <button
            style={{
              ...
              marginRight: index === whiteKeys.length - 1 ? 0 : `${keySpacing}px`,
              borderRadius:
                index === 0
